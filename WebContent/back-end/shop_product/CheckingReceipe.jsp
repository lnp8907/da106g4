			...
			<tbody id="ProductContext">
				<c:forEach var="productvo" items="${list}" begin="<%=pageIndex%>"
					...
					<tr>
						...
						<td><input id="recipeProductid" type="hidden"
							value="${productvo.product_id}">


							<button class="right attached ui button Checkproduct onRproduct"
								${ productstatus [status]=='審核中'? "
	...
	<div id="receipeform" class="border">
		<form METHOD="post" ACTION="Productmanage">
			...
			<div class="ui input focus">
				<input id="recipePrice" name="price" type="text" value="1"
					placeholder="請輸入數字...">
			</div>
			<br>
			<div id="checkRbtn">
				...
				<div class="ui icon input">
					<input class="ui  button cancel-On-Receipe" type="button"
						value="取消"><i class="undo alternate icon"></i>

				</div>

				<div class="ui icon input">
					<input class="ui  button" type="submit" value="送出"><i
						class=" arrow circle right icon"></i> <input type="hidden"
						name="status" value="0"> <input type="hidden"
						name="action" value="onproduct"> <input type="hidden"
						name="onbtn" value="onbtn"> <input type="hidden"
						name="requestURL" value="<%=request.getServletPath()%>"> <input
						type="hidden" id="getproductid" name="product_id" />
				</div>
