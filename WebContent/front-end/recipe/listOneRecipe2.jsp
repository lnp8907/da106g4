...
<%
	RecipeVO_saved recipeVO = (RecipeVO_saved) request.getAttribute("recipeVO"); //EmpServlet.java(Concroller), 存入req的empVO物件
%>
...
<title>詳細食譜內容</title>
<link rel="stylesheet" href="recipeCSS/listOneRecipeCss.css">
<link
	...
	<div class="racipe-main">
		...
		<div class="racipe-detal">
			...
			<div class="racipe-detal-description">
				<h2>食譜介紹</h2>
				<br>
				<p><%=recipeVO.getRecipe_content()%></p>
			</div>
		</div>
		<div class="racipe-detal-info-list">
			<div class="r-detal-info-list-container">
				...
				<div class="r-detal-info-list-descript">
					<table>
						<%
							for (int i = 0; i < ((RecipeVO_saved) recipeVO).getUnits().length; i++) {
						%>
						<tr>
							<th class="ingredient-th"><%=((RecipeVO_saved) recipeVO).getRecipe_ingredients()[i]%></th>
							<td class="ingredient-td"><%=((RecipeVO_saved) recipeVO).getIngredient_amount()[i]%></td>
							<td class="ingredient-td"><%=((RecipeVO_saved) recipeVO).getUnits()[i]%></td>
						</tr>
						<%
							}
						%>
			...
			<div class="r-detal-info-list-container">
				...
				<div class="r-detal-info-list-descript">
					<table>
						<%
							for (int i = 0; i < ((RecipeVO_saved) recipeVO).getRecipe_steps().length; i++) {
						%>
						<tr>
							<th>
								<h4><%=i + 1%></h4>
							</th>
							<td><%=((RecipeVO_saved) recipeVO).getRecipe_steps()[i]%></td>
						</tr>
						<%
							}
						%>
...
<script>
	...
	//         $("#recipeFollow").ajaxForm(function (data) {  
