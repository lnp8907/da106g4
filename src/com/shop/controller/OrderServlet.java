...
//System.out.println("網頁來的金額為:"+total2);
	...
	try {
		...
		System.out.println("傳過來的值地址為:"+dv_address+"會員帳號:"+member_id+"支付方式"+pay_type+"訂單總金額"+total+"訂單狀態:"+order_status);
		OrderService svc=new OrderService();
...
//刪除
		...
		if ("OrderUpdate".equals(action)) {
			...
			try {
				...
				String os = req.getParameter("order_status");				
				 try {
					order_status = Integer.valueOf(os);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					errorMsgs.add("請選擇狀態\n"+"目前訂單狀態:"
							+Nos );
